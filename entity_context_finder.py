...
If context is just the place with a shad or something like that, doesn't store extracted context.
...
desired_num_contexts= 5  # per entity
...
def process_text_files(entity_list, text_files_path, output_path):
    ...
    entity_dict = load_checkpoint('entity_dict.json') or {}
    ...
    finalised_entities = load_checkpoint('finalised_entities.json') or {}
    ...
    # )
    context_sources = load_checkpoint('context_sources.json') or {}
    ...
    finalisation_order = load_checkpoint('finalisation_order.json') or []
    ...
    for filepath in glob.glob(text_files_path + '/**/*.txt', recursive=True):
        ...
        for index, row in tqdm(df.iterrows(), total=len(df), desc="Searching for context"):
            ...
            if current_num_contexts == desired_num_contexts:
                ...
                new_contexts_found = extract_context(text, entity, desired_num_contexts+2)
                ...
                if current_num_contexts == desired_num_contexts:
                    ...
                    save_checkpoint(finalisation_order, 'finalisation_order.json')  # Save progress

        # Save progress after processing each file
        save_checkpoint(entity_dict, 'entity_dict.json')
        save_checkpoint(finalised_entities, 'finalised_entities.json')
        save_checkpoint(context_sources, 'context_sources.json')
        save_checkpoint(finalisation_order, 'finalisation_order.json')
...
# Example usage
entity_list = "PATH/TO/YOUR/DATASET/CSV"  # Replace with your dataset file path
...
with open("./entity_dict.json", 'r', encoding='utf-8') as json_file:
    completed_entity_list = json.load(json_file)

with open("./context_sources.json", 'r', encoding='utf-8') as json_file:
